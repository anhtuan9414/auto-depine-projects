...
async function main() {
  ...
  const getRandomInterval = (minMinutes, maxMinutes) => {
	...
	const checkStatus = async () => {
	  const executeWithDynamicInterval = async () => {
		try {
		  ...
		  if (tryNum <= 5) {
			...
			return; // Exit the function if a retry occurs
		  } else {
	  ...
	  executeWithDynamicInterval(); // Start the initial execution
  };
  ...
  function getRandomIntervalHours(minHours, maxHours) {
  ...
  const getToken = async () => {
	  try {
		...
		console.log(`Next get token execution in: ${(nextInterval / 1000).toFixed(2)} seconds`);
  ...
  let intervalId = setInterval(getToken, getRandomIntervalSeconds(86400, 172800)); // from 24h to 48h
}
